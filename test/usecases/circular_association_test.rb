...
class CircularAssociationTest < Minitest::Test
  ...
  def test_included_option_works_for_serialize
    book = @books.sample
    BookResource.new(book).serialize(included: {book: {authors: :books, genre: :books}})
    assert true # No Error
  end

  def test_included_option_with_nil_value_works_for_serialize
    book = @books.sample
    BookResource.new(book).serialize(included: nil)
    assert true # No Error
  end

  def test_included_option_with_false_value_works_for_serialize
    book = @books.sample
    BookResource.new(book).serialize(included: false)
    assert true # No Error
  end

  def test_included_option_works_for_serializable_hash
    book = @books.sample
    BookResource.new(book).serializable_hash(included: {book: {authors: :books, genre: :books}})
    assert true # No Error
  end

  def test_included_option_works_for_serialize_with_collection
    books = @books.sample(3)
    BookResource.new(books).serialize(included: {book: {authors: :books, genre: :books}})
    assert true # No Error
  end

  def test_included_option_with_nil_end_works_for_serialize
    book = @books.sample
    BookResource.new(book).serialize(included: {book: {authors: nil, genre: nil}})
    assert true # No Error
  end

  def test_included_option_with_array_end_works_for_serialize
    book = @books.sample
    BookResource.new(book).serialize(included: {book: [:authors, :genre]})
    assert true # No Error
  end
end
